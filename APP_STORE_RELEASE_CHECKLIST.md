...
- Submit both subscription products with app version `1.0.1 (4)`.
...
- Release option: use automatic release after approval for `1.0.1 (4)` to minimize the purchase-unavailable window.
...
- iPhone 6.5-inch screenshots: keep the current three screenshots for `1.0.1`.
...
- Do not include the stray draft consumable IAP `gratefulmoments.premium.monthly` in the `1.0.1 (4)` submission; delete it from App Store Connect after explicit confirmation if cleanup is desired.
...
- Confirm App Store Connect growth and marketing sections are intentionally unused for `1.0.1`: app events, custom product pages, product page optimization, promo codes, and Game Center.
...
- Confirm the two App Store Connect subscription product IDs exactly match the app:
  - `smallthanksdiary.premium.monthly`
  - `smallthanksdiary.premium.yearly`
- Confirm both subscriptions are attached to version `1.0.1 (4)` in the In-App Purchases or Subscriptions section before resubmitting.
